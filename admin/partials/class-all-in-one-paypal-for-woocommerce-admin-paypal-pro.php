 ...
 */
class All_In_One_Paypal_For_Woocommerce_Admin_WooCommerce_PayPal_Pro extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        add_action('woocommerce_api_all_in_one_paypal_for_woocommerce_admin_woocommerce_paypal_pro', array($this, 'authorise_3dsecure'));
    ...
    public function init_form_fields() {
        $this->form_fields = array(
            'enabled' => array(
                ...
                'label' => __('Enable PayPal Pro', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'checkbox',
                'description' => '',
                'default' => 'no'
            ),
            'title' => array(
                'title' => __('Title', 'paypal_advance_return_response_url'),
                'type' => 'text',
                'description' => __('This controls the title which the user sees during checkout.', 'all-in-one-paypal-for-woocommerce'),
                'default' => __('Credit card (PayPal)', 'all-in-one-paypal-for-woocommerce'),
                'desc_tip' => true
            ),
            'description' => array(
                ...
                'default' => __('Pay with your credit card via PayPal Website Payments Pro.', 'all-in-one-paypal-for-woocommerce'),
                'desc_tip' => true
            ),
            'testmode' => array(
                'title' => __('Test Mode', 'all-in-one-paypal-for-woocommerce'),
                'label' => __('Enable PayPal Sandbox/Test Mode', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'checkbox',
                'description' => __('Place the payment gateway in development mode.', 'all-in-one-paypal-for-woocommerce'),
                'default' => 'no',
                'desc_tip' => true
            ),
            'api_username' => array(
                ...
                'type' => 'text',
                'description' => __('Get your API credentials from PayPal.', 'all-in-one-paypal-for-woocommerce'),
                'default' => '',
                'desc_tip' => true
            ),
            'api_password' => array(
            ...
            'paypal_pro_paymentaction' => array(
                'title' => __('Payment Action', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'select',
                'description' => __('Choose whether you wish to capture funds immediately or authorize payment only.', 'all-in-one-paypal-for-woocommerce'),
                'default' => 'sale',
                'desc_tip' => true,
                'options' => array(
                    'sale' => __('Capture', 'all-in-one-paypal-for-woocommerce'),
                    'authorization' => __('Authorize', 'all-in-one-paypal-for-woocommerce')
                )
            ),
            'enable_3dsecure' => array(
                'title' => __('3DSecure', 'all-in-one-paypal-for-woocommerce'),
                'label' => __('Enable 3DSecure', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'checkbox',
                'description' => __('Allows UK merchants to pass 3-D Secure authentication data to PayPal for debit and credit cards. Updating your site with 3-D Secure enables your participation in the Verified by Visa and MasterCard SecureCode programs. (Required to accept Maestro)', 'all-in-one-paypal-for-woocommerce'),
                'default' => 'no',
                'desc_tip' => true
            ),
            'centinel_pid' => array(
                'title' => __('Centinel PID', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'text',
                'description' => __('If enabling 3D Secure, enter your Cardinal Centinel Processor ID.', 'all-in-one-paypal-for-woocommerce'),
                'default' => '',
                'desc_tip' => true
            ),
            'centinel_mid' => array(
                'title' => __('Centinel MID', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'text',
                'description' => __('If enabling 3D Secure, enter your Cardinal Centinel Merchant ID.', 'all-in-one-paypal-for-woocommerce'),
                'default' => '',
                'desc_tip' => true
            ),
            'centinel_pwd' => array(
                'title' => __('Transaction Password', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'password',
                'description' => __('If enabling 3D Secure, enter your Cardinal Centinel Transaction Password.', 'all-in-one-paypal-for-woocommerce'),
                'default' => '',
                'desc_tip' => true
            ),
            'liability_shift' => array(
                'title' => __('Liability Shift', 'all-in-one-paypal-for-woocommerce'),
                'label' => __('Require liability shift', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'checkbox',
                'description' => __('Only accept payments when liability shift has occurred.', 'all-in-one-paypal-for-woocommerce'),
                'default' => 'no',
                'desc_tip' => true
            ),
            'send_items' => array(
                'title' => __('Send Item Details', 'all-in-one-paypal-for-woocommerce'),
                'label' => __('Send Line Items to PayPal', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'checkbox',
                'description' => __('Sends line items to PayPal. If you experience rounding errors this can be disabled.', 'all-in-one-paypal-for-woocommerce'),
                'default' => 'no',
                'desc_tip' => true
            ),
            'soft_descriptor' => array(
                'title' => __('Soft Descriptor', 'all-in-one-paypal-for-woocommerce'),
                'type' => 'text',
                'description' => __('(Optional) Information that is usually displayed in the account holder\'s statement, for example your website name. Only 23 alphanumeric characters can be included, including the special characters dash (-) and dot (.) . Asterisks (*) and spaces ( ) are NOT permitted.', 'all-in-one-paypal-for-woocommerce'),
                'default' => '',
                'desc_tip' => true,
            ...
            'debug' => array(
                ...
                'default' => 'no',
                'desc_tip' => true,
                'description' => __('Log PayPal Pro events inside <code>woocommerce/logs/paypal-pro.txt</code>', 'all-in-one-paypal-for-woocommerce'),
            )
        );
    }
    ...
    public function payment_fields() {
        if ($this->description) {
            echo '<p>' . $this->description . ( $this->testmode ? ' ' . __('TEST/SANDBOX MODE ENABLED. In test mode, you can use the card number 4007000000027 with any CVC and a valid expiration date.  Note that you will get a faster processing result if you use a card from your developer\'s account.', 'all-in-one-paypal-for-woocommerce') : '' ) . '</p>';
        }
        $fields = array();
    ...
    public function validate_fields() {
        try {
            ...
            if (2 !== sizeof($card_expiry)) {
                throw new Exception(__('Card expiration date is invalid', 'all-in-one-paypal-for-woocommerce'));
            }
            ...
            if (!ctype_digit($card_cvc)) {
                throw new Exception(__('Card security code is invalid (only digits are allowed)', 'all-in-one-paypal-for-woocommerce'));
            }
            if (!ctype_digit($card_exp_month) || !ctype_digit($card_exp_year) || $card_exp_month > 12 || $card_exp_month < 1 || $card_exp_year < date('y')) {
                throw new Exception(__('Card expiration date is invalid', 'all-in-one-paypal-for-woocommerce'));
            }
            if (empty($card_number) || !ctype_digit($card_number)) {
                throw new Exception(__('Card number is invalid', 'all-in-one-paypal-for-woocommerce'));
            }
            return true;
        } catch (Exception $e) {
    ...
    public function process_payment($order_id) {
        ...
        if ($this->enable_3dsecure) {
            ...
            WC()->session->set('Centinel_TermUrl', WC()->api_request_url('All_In_One_Paypal_For_Woocommerce_Admin_WooCommerce_PayPal_Pro', true));
            ...
            if (WC()->session->get('Centinel_ErrorNo') == 0) {
                if (WC()->session->get('Centinel_Enrolled') == 'Y') {
                    ...
                    <html>
                        ...
                        <body>
                            <form name="frmLaunchACS" id="3ds_submit_form" method="POST" action="<?php echo WC()->session->get("Centinel_ACSUrl"); ?>">
                                ...
                                ?>">
                                <noscript>
                                <div class="woocommerce_message"><?php _e('Processing your Payer Authentication Transaction', 'all-in-one-paypal-for-woocommerce'); ?> - <?php _e('Please click Submit to continue the processing of your transaction.', 'all-in-one-paypal-for-woocommerce'); ?>  <input type="submit" class="button" id="3ds_submit" value="Submit" /></div>
                                </noscript>
                            </form>
                            <script>
                    ...
                    wc_add_notice(__('Authentication unavailable. Please try a different payment method or card.', 'all-in-one-paypal-for-woocommerce'), 'error');
                    return;
                } else {
                ...
                wc_add_notice(__('Error in 3D secure authentication: ', 'all-in-one-paypal-for-woocommerce') . WC()->session->get('Centinel_ErrorNo'), 'error');
                return;
            }
        }
        ...
        try {
            ...
            if ($this->liability_shift) {
                if ($eci_flag == '07' || $eci_flag == '01') {
                    $order->update_status('failed', __('3D Secure error: No liability shift', 'all-in-one-paypal-for-woocommerce'));
                    throw new Exception(__('Authentication unavailable.  Please try a different payment method or card.', 'all-in-one-paypal-for-woocommerce'));
                }
            }
            if ($error_no == "0") {
                if (( $pa_res_status == "Y" || $pa_res_status == "A" || $pa_res_status == "U") && $sig_verification == "Y") {
                    ...
                    $order->update_status('failed', sprintf(__('3D Secure error: %s', 'all-in-one-paypal-for-woocommerce'), $error_desc));
                    throw new Exception(__('Payer Authentication failed.  Please try a different payment method.', 'all-in-one-paypal-for-woocommerce'));
                }
            } else {
                $order->update_status('failed', sprintf(__('3D Secure error: %s', 'all-in-one-paypal-for-woocommerce'), $error_desc));
                throw new Exception(__('Error in 3D secure authentication: ', 'all-in-one-paypal-for-woocommerce') . $error_desc);
            }
        } catch (Exception $e) {
            wc_add_notice($e->getMessage(), 'error');
        }
    ...
    public function do_payment($order, $card_number, $card_type, $card_exp_month, $card_exp_year, $card_cvc, $card_start_month, $card_start_year, $centinelPAResStatus = '', $centinelEnrolled = '', $centinelCavv = '', $centinelEciFlag = '', $centinelXid = '') {
        ...
        try {
            ...
            $response = wp_remote_post($url, array(
                ...
                'body' => apply_filters('all-in-one-paypal-for-woocommerce_request', $post_data, $order),
                'timeout' => 70,
                'user-agent' => 'all-in-one-paypal-for-woo',
                'httpversion' => '1.1'
            ));
            ...
            if (empty($response['body'])) {
                ...
                throw new Exception(__('Empty Paypal response.', 'all-in-one-paypal-for-woocommerce'));
            }
            parse_str($response['body'], $parsed_response);
            ...
            switch (strtolower($parsed_response['ACK'])) {
                ...
                case 'successwithwarning':
                    ...
                    if ($details && strtolower($details['PAYMENTSTATUS']) === 'pending' && strtolower($details['PENDINGREASON']) === 'authorization') {
                        ...
                        $order->update_status('on-hold', sprintf(__('PayPal Pro charge authorized (Charge ID: %s). Process order to take payment, or cancel to remove the pre-authorization.', 'all-in-one-paypal-for-woocommerce'), $txn_id));
                        $order->reduce_order_stock();
                    } else {
                        $order->add_order_note(sprintf(__('PayPal Pro payment completed (Transaction ID: %s, Correlation ID: %s)', 'all-in-one-paypal-for-woocommerce'), $txn_id, $correlation_id));
                ...
                default:
                    ...
                    $order->update_status('failed', sprintf(__('PayPal Pro payment failed (Correlation ID: %s). Payment was rejected due to an error: ', 'all-in-one-paypal-for-woocommerce'), $parsed_response['CORRELATIONID']) . '(' . $parsed_response['L_ERRORCODE0'] . ') ' . '"' . $error_message . '"');
                    throw new Exception($error_message);
                    break;
            }
        } catch (Exception $e) {
            wc_add_notice('<strong>' . __('Payment error', 'all-in-one-paypal-for-woocommerce') . '</strong>: ' . $e->getMessage(), 'error');
            return;
        }
    }
